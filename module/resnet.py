...
class ResnetBlock2D(nn.Module):
    ...
    def forward(
        ...
        temb: Optional[torch.Tensor] = None, # XXX: None for Subject-driven generation
        *args,
        **kwargs
    ) -> torch.Tensor:
        if len(args) > 0 or kwargs.get("scale", None) is not None:
        ...
        if self.time_emb_proj is not None and temb is not None:  # XXX: None for Subject-driven generation
