...
from .attention_processor import FusedAttnProcessor2_0, Attention
...
class UNet2DConditionModel(
    ...
    def __init__(
        ...
        self.condition_conv_in = nn.Conv2d(
            4, block_out_channels[0], kernel_size=conv_in_kernel, padding=conv_in_padding
        )  # XXX For Condition embedding
    ...
    def clear_kv_cache(self):
        for attn_processor in self.attn_processors.values():
            if hasattr(attn_processor, "kv_cache"):
    ...
    def fuse_qkv_projections(self):
        ...
        for module in self.modules():
            if isinstance(module, Attention):
                ...
                module.set_processor(FusedAttnProcessor2_0())
    ...
    def forward(
        ...
        # 1.1 Condition embedding (XXX: For Subject-driven generation)
        ...
        if class_labels is None:
            ...
            sample = self.condition_conv_in(sample)
        ...
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                sample, res_samples = downsample_block(hidden_states=sample, temb=emb, cache_kv=cache_kv)
                if is_adapter and len(down_intrablock_additional_residuals) > 0:
        ...
        if self.mid_block is not None:
            if hasattr(self.mid_block, "has_cross_attention") and self.mid_block.has_cross_attention:
                ...
                sample = self.mid_block(sample, emb, cache_kv=cache_kv)
